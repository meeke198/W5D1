...
SELECT
movies.title,
COUNT(DISTINCT castings.actor_id) AS count_actor
FROM
movies
JOIN 
castings ON castings.movie_id = movies.id
...
def colleagues_of_garfunkel
  ...
  actors
  JOIN
  castings ON castings.actor_id = actor.id
  JOIN
  movies ON movies.id = castings.actor_id
  WHERE
  movies.id IN (
    SELECT
    movies.id
    ...
    castings ON castings.actor_id = actor.id
    JOIN
    actors ON actors.id = castings.movie_id
    WHERE
